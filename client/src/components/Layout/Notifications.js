...
class Notifications extends Component {
    ...
    render() {
        ...
        if (notifications && notifications.length > 0) {
            let notReadNotificationsNumber  = notifications.filter(not => not.read === false).length;
            notReadNotificationsNumber > 0
                ? notificationsIcon = (
                    <Badge badgeContent={notReadNotificationsNumber} color="secondary">
                        <NotificationsIcon />
                    </Badge>
                ) : (
        ...
        let notificationsMarkup = 
            notifications && notifications.length > 0 ? (
                notifications.map(not => {
                    ...
                    return (
                        <MenuItem key={not.createdAt} onClick={this.handleClose} component={Link} to={`/users/${not.recipient}/post/${not.postId}`}>
                            {icon}
